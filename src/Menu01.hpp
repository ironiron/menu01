 ...
 struct list_base
{
    ...
    virtual unsigned int Get_max(void) = 0;
...
{
    constexpr  menu_list(const std::string_view (&name), const std::string_view (&s)[M],T_Par * parent_menu=nullptr,unsigned int index=0) :
            _name(name),
            _items(
    ...
    list_base* Get_child(unsigned int index)
    {
        return child[index];
    }
    ...
    virtual unsigned int Get_max(void)
    {
        return _items.size();
    }
    virtual unsigned int Get_parent_index(void)
    {
        return _index;
    }
    list_base* Get_parent(void)
    {
        if constexpr (std::is_same_v<const std::nullptr_t, T_Par> == false)
        {
...
{
public:
    Menu01( T &&root_page) : _list(root_page),_current_list(_list),_parent_list(nullptr)
    {
    }
    virtual ~Menu01()
    {
